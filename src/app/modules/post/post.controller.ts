...
export const createPost = request.createOne(Post);
...
// banner post
export const getBannerPost = catchAsync(async (req, res) => {
  const posts = await Post.find({ isDeleted: false })
    .populate({
      path: 'author',
      select: 'name email profilePic isVerified',
...
export const updatePost = request.updateOne(Post);
...
export const deletePost = catchAsync(async (req, res, next) => {
  ...
  if (!post) {
    return next(
      new AppError(httpStatus.NOT_FOUND, 'Post not found or already deleted'),
    );
  }
